  ...
  //'kburkette','ewardenburg','mofo','ccastillo')
       ...
       public  ProcessManager()
        ...
        private void LoadYesterdayHash()
        {
            ...
            if (debug)  lm.Write("ProcessManager/LoadYesterdayHash:");
            int reqItemID = 0;
            int reqNo = 0;
            int reqLine = 0;
            string login = "";
            ...
            {
                lm.Write("ProcessManager/LoadYesterdayHash:  " + ex.Message);
            }
        }
        ...
        {//use the list of reqItemIDs from the ReqItemStatus table and loaded in the LoadYesterdayHash method
            ...
            if (debug)  lm.Write("ProcessManager/LoadChangedDates");
        ...
        private void CheckStatus()
        {
             ...
             */
            if (debug)  lm.Write("ProcessManager/CheckStatus");
            int reqItemID = 0;
            DateTime chngDate = new DateTime();
            string newStatus = "";
            ...
            try
            {
                if (dsCurrentChangeDates.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        if (StatusFilter(drow[1].ToString()))
                        {
                            ...
                            if (drow[0] != DBNull.Value && drow[2] != DBNull.Value)
                            {
                                ...
                                chngDate = Convert.ToDateTime(drow[2]);
                                //     if (debug) lm.Write("ProcessManager/CheckStatus reqItemID = " + reqItemID + "  STAT = " + newStatus);
            ...
            {
                lm.Write("ProcessManager/CheckStatus:  " + ex.Message);
            }
        }

        private bool StatusFilter(string stat)
        {
            bool goodToGo = false;
            if (stat == "On Order" || stat == "Pend Apvl")
            {
                goodToGo = true;
            }
            return goodToGo;
        ...
        {/*
          ...
          * */
            if (debug)  lm.Write("ProcessManager/OrganizeByLogin");
        ...
        private void PrepareOutput()
        {
            ...
            {
                lm.Write("ProcessManager/PrepareOutput:  ERROR:  " + ex.Message);
            }
        }
        ...
        {
            if (debug) lm.Write("ProcessManager/UpdateStatus");
        ...
        {
            if (debug) lm.Write("ProcessManager/StoreNewReqs");
